        ...
        if key == "source":
            for node in j2.get(key):
                ...
                if node['type'].lower().strip() == 'kafka':
                    ...
                    with_dict = {'connector': node['type'], 'topic': node['config']['table'],
                                 'bootstrap.servers': node['config']['bootstrap.servers'],
                                 'group.id': node['config']['group.id'], 'format': "json"}
                if node['type'].lower().strip() == 'es' or node['type'].lower().strip() == 'elasticsearch':
                    ...
                    with_dict = {"connector": 'elasticsearch-7', "hosts": node['config']["hosts"],
